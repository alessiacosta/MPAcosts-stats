...
## Trying a Bayesian with subcategories for establishment and maintenance

```{r}
#loading data
tsubs <- read_csv("../Data/tsubs.csv")
View(tsubs)

#data preparation
tsubs = tsubs |> mutate(Protection_level=factor(Protection_level, 
                                                levels=c("Fully protected", "Partially protected")), # this will order them rather than going into alphabetical order 
                        Cost_class=factor(Cost_class, 
                                          levels=c("Establishment", "Maintenance", "Research", "Planning", "Community outreach", "Management", 
...
## Data exploration to see what distribution I can use in the model
#looking at the distribution for cost/km2
hist(tsubs$Cost_km, breaks=75)
#looking at the distribution of costs/km2 divided by cost category
boxplot(Cost_km~Protection_level, data=tsubs)

#Visualisiing data
ggplot(data=tsubs, aes(x=Cost_km)) +
  geom_boxplot()
#checking for linearity -> equal variance (no relationship between mean and variance)
ggplot(data=tsubs, aes(x=GDP_pc, y=Cost_km)) +
  geom_point() +
  geom_smooth()
#better way for doing this and seeing a straight line in the data 
ggplot(data=tsubs, aes(x=GDP_pc, y=Cost_km)) +
...
#boxplot
tsubs |> ggplot(aes(x=Cost_class, y=Cost_km, fill=Protection_level)) +
  geom_boxplot() +
  geom_point(aes(color=Protection_level), 
             position =position_dodge(width = 0.75))
...
#model formula
form2 = bf(Cost_km ~ scale(GDP_pc) + Protection_level + Management + Administration + Enforcement + Research + Planning + Community_outreach, family=Gamma(link='log'))
  
#looking at priors needed
get_prior(form2, tsubs)
...
# running the model with both priors and data
tsubs.brm2 = tsubs.brm1 |> 
  update(sample_prior = 'yes', refresh=0)

#Assess priors and posteriors
tsubs.brm2 |> SUYR_prior_and_posterior()

### MCMC sampling diagnostics
#trace plot 
tsubs.brm2$fit |> stan_trace()

#border correlarion
tsubs.brm2$fit |> stan_ac()

#chain convergence
tsubs.brm2$fit |> stan_rhat()

#effective sample size
tsubs.brm2$fit |> stan_ess() # there are no issues here
...
## Comparing maintenance costs with establishment costs 
```{r}
mesubs <- read_csv("../Data/mesubs.csv")
#data preparation
mesubs = mesubs |> mutate(Protection_level=factor(Protection_level, 
                                                levels=c("Fully protected", "Partially protected")), # this will order them rather than going into alphabetical order 
                        Management=factor(Management, 
                                          levels=c(0,1)), 
                        Administration=factor(Administration, 
...
source('helperFunctions.R')
#checking distribution 
ggplot(data=mesubs, aes(x=Cost_km)) +
  geom_boxplot()
#checking for linearity -> equal variance (no relationship between mean and variance)
ggplot(data=mesubs, aes(x=GDP_pc, y=Cost_km)) +
  geom_point() +
  geom_smooth()
#better way for doing this and seeing a straight line in the data 
ggplot(data=mesubs, aes(x=GDP_pc, y=Cost_km)) +
...
#looking at distribution 
#looking at the distribution for cost/km2
hist(mesubs$Cost_km, breaks=75)

#boxplot
mesubs |> ggplot(aes(x=Cost_class, y=Cost_km, fill=Protection_level)) +
  geom_boxplot() +
  geom_point(aes(color=Protection_level), 
             position =position_dodge(width = 0.75))

#plots with no outliers
mesubs_filtered <- mesubs |> 
  group_by(Cost_class) |> 
  filter(
    Cost_km >= quantile(Cost_km, 0.25) - 1.5 * IQR(Cost_km) &
    Cost_km <= quantile(Cost_km, 0.75) + 1.5 * IQR(Cost_km)
...
View(mesubs)
mesubs_filtered |> 
  group_by(Protection_level, Administration, Management, Enforcement, GDP_pc, Establishment) |>  
  summarise(Median=median(log(Cost_km)), MAD=mad(log(Cost_km)))
...
#model formula
form = bf(Cost_km ~ scale(GDP_pc) + Protection_level + Management + Administration + Enforcement + Establishment, family=Gamma(link='log'))
default_prior(form, data=mesubs)
#getting standard priors
get_prior(form, data=mesubs)

# running the model with priors only
mesubs.brm1 = brm(form, 
               data=mesubs, 
               prior=priors,
               sample_prior='only', 
               iter= 5000, 
...
# running the model with both priors and data
mesubs.brm2 = mesubs.brm1 |> 
  update(sample_prior = 'yes', refresh=0)

#Assess priors and posteriors
mesubs.brm2 |> SUYR_prior_and_posterior()

### MCMC sampling diagnostics
#trace plot 
mesubs.brm2$fit |> stan_trace()

#border correlarion
mesubs.brm2$fit |> stan_ac()

#chain convergence
mesubs.brm2$fit |> stan_rhat()

#effective sample size
mesubs.brm2$fit |> stan_ess() # there are no issues here

### Model validation

#probability check
mesubs.brm2 |> pp_check(type='dens_overlay', ndraws=100)
#not the best fit it could be 

#sim residuals 
mesubs.resids  = make_brms_dharma_res(mesubs.brm2, integerResponse= TRUE)
testUniformity(msubs.resids)
#simulated residuals are not too bad, acceptable

#check no patterns in the residuals 
plotResiduals(mesubs.resids, quantreg = TRUE)
#red lines that there might be some trends occurring, because the line is high and it decreases. 
#without lines we would have not noticed the trends
