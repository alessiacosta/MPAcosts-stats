...
nesize = 
  ...
  ylim(0, 1.0e+06) + # setting the y limits to make it more readable
  scale_y_continuous(labels = label_scientific()) +
  labs(
    x = "lnSize (ln(km²))",
    y = "Estmated establishment costs (cost/km²)"
  )
...
form = bf(sCost_km ~ 0 + Administration + Enforcement + Management, family=gaussian)
# mean for each group and mads for each group for our priors
msubs |> 
  summarise(Median=median(sCost_km), MAD=mad(sCost_km))
...
priors =# prior(normal(8, 3), class='Intercept') +
  prior(normal(0,0.5), class='b') #within brackets add lb=0
...
msubs.brm2 |>
  ...
  plot(points=TRUE) |> 
  wrap_plots()
#plot cost_km vs l size with a trend line
# Remove the largest value of Size
#msubs <- msubs |> filter(Size < max(Size, na.rm = TRUE))
#msubs <- msubs |> filter(Size < max(Size, na.rm = TRUE))
msubs <- msubs |> filter(Size < max(Size, na.rm = TRUE))
msubs <- msubs |> filter(Size < max(Size, na.rm = TRUE))
ggplot(msubs, aes(x=Size, y=Cost_km)) +
  geom_point() +
  geom_smooth(method="glm", method.args = list(family = gaussian(link = "log")), 
              formula = y ~ x, se = TRUE) +
  scale_y_log10() + 
  labs(x="lnSize (km²)", y="Maintenance cost/km² p.a.") +
  theme_bw() +
  theme(text = element_text(family = "serif"))
```
...
msubs.brm2 |> 
  as_draws_df() |>  
  dplyr::select(matches("^b_.*")) |> 
  summarise_draws(median=~exp(median(.x)+2.42/2), 
                  HDInterval::hdi,
                  rhat, 
                  ess_bulk,
                  Pl=~mean(.x<0), 
...
```{r}
# Step 1: Build raw newdata
#prediction of the maintenance costs/km2 per year
pred_size2 <- emmeans(
  msubs.brm2,
  ~ Size | Protection_level,
  at = list(Size = seq(min(msubs$Size), max(msubs$Size), length.out = 100),
