...
#pred_size <- emmeans(
  ...
  #gather_emmeans_draws() |>               # Get posterior draws in tidy format
  #mutate(.value = exp(.value)) |>         # Back-transform from log scale
  #group_by(lSize) |>                      # Summarise for each lSize
  #summarise(
    #median = median(.value),
    #lower.HPD = hdi(.value)[1],
    #upper.HPD = hdi(.value)[2]
  #) |> 
  #ungroup()
...
#plot prediction
 ...
 #Combine and export the inset
png("years.png", width = 1500, height = 1000, res = 300) 
#final_plot <- 
  nyears +
  inset_element(
    lyears,
...
msubs<- msubs |> 
  ...
  mutate(lSize=log(Size)) |> 
  mutate(lGDP_pc=log(GDP_pc)) #standardising the size and gdp to make it easier to interpret the results
#model formula 
...
form = bf(lCost_km ~ scale(lSize) + scale(lGDP_pc) + Protection_level, family=gaussian)
# mean for each group and mads for each group for our priors
msubs |> 
...
nesize_pl <- ggplot() +
  ...
  geom_line(data = mubse, aes(x = lSize, y = median, color = "Partially protected areas"), size = 0.8) +
  geom_ribbon(data = mubse, aes(x = lSize, ymin = lower.HPD, ymax = upper.HPD, fill = "Partially protected areas"), alpha = 0.2) +
  # Customize scales
  scale_color_manual(values = c("Fully protected areas" = "#1b9e77", "Partially protected areas" = "#d95f02")) +
  scale_fill_manual(values = c("Fully protected areas" = "#1b9e77", "Partially protected areas" = "#d95f02")) +
...
lesize_pl = 
  ggplot() +
  geom_line(data = fpe, aes(x = lSize, y = median), color = "#1b9e77", size = 0.8) +
  geom_line(data = mubse, aes(x = lSize, y = median), color = "#d95f02", size = 0.8) +
  geom_ribbon(data = fpe, aes(x = lSize, ymin = lower.HPD, ymax = upper.HPD),
              fill = "#1b9e77", alpha = 0.2, color = NA) +
  geom_ribbon(data = mubse, aes(x = lSize, ymin = lower.HPD, ymax = upper.HPD),
              fill = "#d95f02", alpha = 0.2, color = NA) +
  geom_point(data = esubs, aes(x = lSize, y = Cost_km, color = Protection_level),
             show.legend = FALSE) +
  scale_color_manual(
    values = c("Fully protected" = "#1b9e77", "Partially protected" = "#d95f02"),
...
#nmsize_pl = 
  ggplot() +
  geom_line(data = fp, aes(x = lSize, y = median), color = "#1b9e77", size = 0.8) +
  geom_line(data = mubs, aes(x = lSize, y = median), color = "#d95f02", size = 0.8) +
  geom_ribbon(data = fp,aes(x = lSize, ymin = lower.HPD, ymax = upper.HPD), fill = "#1b9e77", alpha = 0.2, color = NA) +
  geom_ribbon(data = mubs,aes(x = lSize, ymin = lower.HPD, ymax = upper.HPD), fill = "#d95f02", alpha = 0.2, color = NA) +
  #geom_point(data = msubs, aes(x = lSize, y = Cost_km, color = Protection_level), show.legend = NA) +
  scale_color_manual(values = c("Fully protected" = "#1b9e77", "Partially protected" = "#d95f02"), 
                     labels = c("Fully protected" = "Fully protected areas", "Partially protected" = "Multi-use areas")) +
  scale_y_continuous(labels = scales::label_scientific(), limits = c(0, 1.0e+06)) +
  theme_bw() +
  theme(text = element_text(family = "serif")) +
  labs(
    x = "lnSize (km²)",
...
pred_draws_GDP <- emmeans(
  ...
  ~ lGDP_pc,
  at = list(
   lGDP_pc = seq(min(msubs$lGDP_pc), max(msubs$lGDP_pc), length.out = 100),
...
#neGDP_pc = 
  ggplot(pred_GDP, aes(x = lGDP_pc, y = median)) +
  geom_line(size = 0.8) +  # thinner line
  geom_ribbon(aes(ymin = lower.HPD, ymax = upper.HPD), alpha = 0.2, color = NA) +
  ...
  scale_y_continuous(
    labels = label_scientific(),
    limits = c(0, 4.0e+04)  # forces the axis to go up to 1e6
  ) +
  labs(
 ...
 ggplot(pred_GDP, aes(x = lGDP_pc, y = median)) +
  geom_line(size = 0.8) +  # thinner line
  geom_ribbon(aes(ymin = lower.HPD, ymax = upper.HPD), alpha = 0.2, color = NA) +
  geom_point(data=msubs, aes(x=lGDP_pc, y=Cost_km)) +
  theme_bw() +
  theme(text = element_text(family = "serif")) +
  scale_y_log10() +
  labs(
