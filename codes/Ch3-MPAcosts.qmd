...
#esubs <- within(esubs, {
    #csize <- as.numeric(scale(Size, scale = FALSE))
    #cgdp <- as.numeric(scale(GDP_pc, scale = FALSE))
#})
View(esubs)
```
...
msubs_filtered <- msubs |> 
  group_by(Cost_class) |> 
  filter(
    Cost_km >= quantile(Tcost, 0.25) - 1.5 * IQR(Tcost) &
    Cost_km <= quantile(Tcost, 0.75) + 1.5 * IQR(Tcost)
  ) |> 
  ungroup()


# Plot without outliers
ggplot(msubs_filtered, aes(x = Cost_class, y = Tcost, fill = Protection_level)) +
  geom_boxplot() +
...
View(msubs)
msubs |> 
  group_by(Protection_level, Administration, Management, Enforcement, Size) |>  
  summarise(Median=median(log(Cost_km)), MAD=mad(log(Cost_km)))
...
#model formula
form = bf(Tcost ~ scale(Size) + Protection_level + Management + Administration + Enforcement, family=Gamma(link='log'))
