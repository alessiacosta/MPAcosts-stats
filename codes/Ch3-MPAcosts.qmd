...
form1 = bf(lCost_km ~ lSize + Years + lSize*Protection_level, family = lognormal)
...
form1 = bf(lCost_km ~ lSize, family = lognormal)
...
esubs.brm1 = brm(form1, 
               ...
               backend = "cmdstanr")
# saving the model 
saveRDS(esubs.brm1, "esubs_brm1.rds")
...
form = bf(lCost_km ~ lSize + GDP_pc + lSize*Protection_level, family=gaussian)
# backward deletion -  removed the GDP per capita
form = bf(lCost_km ~ lSize + lSize*Protection_level, family=gaussian)
# backward deletion - removed protection level 
form = bf(lCost_km ~ lSize, family=gaussian)
...
# export the dataset
write_csv(pred_size2, "../Data/pred_size_m.csv")
#prediction plot
nmsize = 
...
pred_size <- pred_draws_e |>
  ...
  mutate(
    median = exp(estimate),
    lower = exp(estimate - 1.96 * se), # t-value = 1.96 for 95% confidence interval
...
nmsize_pl = 
  ...
  geom_ribbon(data = fp, aes(x = Size, ymin = lower.HPD, ymax = upper.HPD, fill = "Fully protected areas"), alpha = 0.2) +
  # Partially protected areas
  geom_line(data = mubs, aes(x = Size, y = median, color = "Multi-use areas"), size = 0.8) +
  geom_ribbon(data = mubs, aes(x = Size, ymin = lower.HPD, ymax = upper.HPD, fill = "Multi-use areas"), alpha = 0.2) +
  # Customize scales
  scale_color_manual(values = c("Fully protected areas" = "#0072B2", "Multi-use areas" = "#E69F00")) +
  scale_fill_manual(values = c("Fully protected areas" = "#0072B2", "Multi-use areas" = "#E69F00")) +
  # Scientific notation for y-axis
...
lmsize_pl <- 
  ...
  geom_ribbon(data = fp, aes(x = Size, ymin = lower.HPD, ymax = upper.HPD),
              fill = "#0072B2", alpha = 0.2, color = NA) +
  geom_ribbon(data = mubs, aes(x = Size, ymin = lower.HPD, ymax = upper.HPD),
              fill = "#E69F00", alpha = 0.2, color = NA) +
