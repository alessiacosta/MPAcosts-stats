...
#prediction plot
msize = ggplot(pred_size, aes(x = Size, y = response, color = Protection_level, fill = Protection_level)) +
  geom_line(size = 0.8) +
  geom_ribbon(aes(ymin = lower.HPD, ymax = upper.HPD), alpha = 0.2, color = NA) +
  scale_y_log10() +
  theme_bw() +
...
#### combining plots
```{r}
png("combined_cost_plot.png", width = 2000, height = 800, res = 300)  # adjust as needed
esize + msize + plot_layout(guides = 'collect')
dev.off()
```
