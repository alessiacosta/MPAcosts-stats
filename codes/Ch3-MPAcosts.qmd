...
form1 = bf(lCost_km ~ lSize + Years + lSize*Protection_level, family=gaussian)
#backward deletion - removed the interaction
form1 = bf(lCost_km ~ lSize + Years + Protection_level, family=gaussian)
# model with size only
form1 = bf(lCost_km ~ lSize, family=gaussian)
...
#making a subset for each cost class, maitenance, management, administration and enforcement 
msubs <- msubs |>  
...
pred_draws_e <- emmeans(
  ...
  at = list(
    lSize = seq(min(esubs$lSize), max(esubs$lSize), length.out = 10),
...
pred_size <- pred_draws_e |> 
  ...
  map_dfr(function(df) { 
    lower <- quantile(df$.value, 0.025) 
    upper <- quantile(df$.value, 0.975) 
    tibble( 
      lSize = unique(df$lSize), 
      Size = exp(unique(df$lSize)), 
      Protection_level = unique(df$Protection_level), 
      median = median(df$.value), lower = lower, upper = upper ) })
```
...
nesize_pl <- 
  ...
  geom_ribbon(data = fpe, aes(x = Size, ymin = lower.HPD, ymax = upper.HPD, fill = "Fully protected areas"), alpha = 0.2) +
  # Partially protected areas
  geom_line(data = mubse, aes(x = Size, y = median, color = "Multi-use areas"), size = 0.8) +
  geom_ribbon(data = mubse, aes(x = Size, ymin = lower.HPD, ymax = upper.HPD, fill = "Multi-use areas"), alpha = 0.2) +
  # Customize scales
...
lesize_pl <- 
  ...
  geom_ribbon(data = fpe, aes(x = Size, ymin = lower.HPD, ymax = upper.HPD),
              fill = "#0072B2", alpha = 0.2, color = NA) +
  geom_ribbon(data = mubse, aes(x = Size, ymin = lower.HPD, ymax = upper.HPD),
