...
form1 = bf(lCost_km ~ scale(lSize) + scale(Years) + scale(lSize)*Protection_level, family=gaussian)
#backward deletion - removed the interaction
form1 = bf(lCost_km ~ scale(lSize) + scale(lSize)*Protection_level, family=gaussian)
# model with size only
form1 = bf(lCost_km ~ scale(lSize), family=gaussian)
...
form = bf(lCost_km ~ scale(lSize) + scale(GDP_pc) + scale(lSize)*Protection_level, family=gaussian)
# backward deletion -  removed the interaction
form = bf(lCost_km ~ scale(lSize) + scale(GDP_pc) + Protection_level, family=gaussian)
# backward delition - removed GDP per capita
form = bf(lCost_km ~ scale(lSize) + Protection_level, family=gaussian)
# backward deletion - removed protection level 
form = bf(lCost_km ~ scale(lSize), family=gaussian)
...
msubs.brm1 |> 
  ...
  summarise_draws(median=~exp(median(.x)), 
                  ...
                  Pl = ~mean(.x<1), 
                  Pg = ~mean(.x>1), 
                  se = ~sd(exp(.x)))   # calculate teh standard error
...
neGDP_pc <- 
  ggplot(pred_GDP, aes(x = GDP_pc, y = median)) +
    geom_line(size = 0.8, linetype = "dashed", color = "#8C1D40") +
...
leGDP_pc <- 
 ggplot(pred_GDP, aes(x = GDP_pc, y = median)) +
  geom_line(size = 0.8, linetype = "dashed", color = "#8C1D40") +  # thinner line
  geom_ribbon(aes(ymin = lower.HPD, ymax = upper.HPD), fill = "#800020", alpha = 0.2, color = NA) +
