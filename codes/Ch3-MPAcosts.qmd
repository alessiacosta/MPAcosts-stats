...
# calculating the partial R2
# 1, fitting a model with posteriors only
esubs.brm3 <- brm(
  form1,
  data = esubs,
  prior = priors,
  ...
  control = list(adapt_delta=0.99, max_treedepth=29),
  backend = "cmdstanr")
# 2. run the partial r2 code for size
partial_r2 <- bayes_R2(esubs.brm3, summary = FALSE) - 
              bayes_R2(update(esubs.brm3, formula. = . ~ . - scale(lSize) - scale(lSize):Protection_level), summary = FALSE)
tidybayes::median_hdci(partial_r2) # 52.5% of variance
# same calculation for years
partial_r3 <- bayes_R2(esubs.brm3, summary = FALSE) - 
              bayes_R2(update(esubs.brm3, formula. = . ~ . - scale(Years)), summary = FALSE)
tidybayes::median_hdci(partial_r3)
# same for degrees of protection 
partial_r4 <- bayes_R2(esubs.brm3, summary = FALSE) - 
              bayes_R2(update(esubs.brm3, formula. = . ~ . - Protection_level), summary = FALSE)
tidybayes::median_hdci(partial_r4)
...
nesize = 
  ggplot(pred_size, aes(x = lSize, y = response)) +
  geom_line(size = 0.8) +  # thinner line
  geom_ribbon(aes(ymin = lower.HPD, ymax = upper.HPD), alpha = 0.2, color = NA) +
  theme_bw() +
...
msubs.brm3 <- brm(
  ...
  backend = "cmdstanr")
# 2. run the partial r2 code for size
partial_r2m <- bayes_R2(msubs.brm3, summary = FALSE) - 
              bayes_R2(update(msubs.brm3, formula. = . ~ . - scale(lSize) - scale(lSize):Protection_level), summary = FALSE)
tidybayes::median_hdci(partial_r2m) # 29% variance
```
...
nesize_pl <- 
  ...
  scale_y_continuous(
    ...
    limits=c(0, 1.e06)) +
  scale_x_continuous(
    labels = function(x) parse(text = paste0("e^", x))) +  # this makes a e to the power label and "parse" makes it a superscript format
  # Apply theme
  theme_bw() +
  theme(text = element_text(family = "serif"), 
        #panel.grid.major = element_blank(),
  ...
  labs(
    ...
    title = " ") +
  coord_cartesian(clip="off") +
  annotate("text", x=-Inf, y=Inf, label="A", hjust=1.7, vjust=0.3, size=5, fontface="bold", family='serif')
#transformed axis
lesize_pl = 
  ggplot() +
...
nmsize_pl = 
  ...
  labs(
    ...
    title = " ") +
  coord_cartesian(clip="off") +
  annotate("text", x=-Inf, y=Inf, label="B", hjust=1.7, vjust=0.3, size=5, fontface="bold", family='serif')
nmsize_pl
